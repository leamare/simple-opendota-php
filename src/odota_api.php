...
class OpenDotaException extends \Exception { }
...
class odota_api {
   ...
   * @param bool $throw = false Throw Exception on request error
   */
  function __construct($cli_report_status=\false, $hostname="", $cooldown=0, $api_key="", $throw=false) {
    if (!empty($hostname))
  ...
  private function get($url, $data = []) {
    ...
    if ( $this->report_status ) {
      if ( !empty($response) )
        ...
        echo("\n[E] OpenDotaPHP: Error while trying to reach the host\n");
    }
